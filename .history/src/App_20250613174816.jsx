...
function App() {
  ...
  if (flipped){
    text = Card_DATA[index].answer
  } else {
    text = Card_DATA[index].question
  }
  const image = Card_DATA[index].image
  const title = Card_DATA[index].category
  const text = flipped ? Card_DATA[index].answer : Card_DATA[index].question
  ...
  return (
    <>
      <Card text="" onClick={() => flipCard(index)}/>
