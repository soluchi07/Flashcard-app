...
function App() {
  ...
  const flipCard = () => {
    ...
    console.log("Card text:", card_text);
    // nextCard();
    // setFlipped(false);
  }
  ...
  return (
    <>
        ...
        <h2>Let's help you pass for your next CS132 final!</h2>
        <h3>Card {index + 1} of {Card_DATA.length}</h3>
        <div onClick={flipCard} onKeyDown={flipCard} className={`card-container ${flipped ? "flipped" : ""}`}>
            <Card text={card_text} category={category} image={image}/>
        </div>      
        <button className="prev" onClick={prevCard}>Prev Card</button>
