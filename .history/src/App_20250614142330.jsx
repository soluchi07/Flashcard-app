...
function App() {
  ...
  const nextCard = () => {
    setIndex((index) => (index + 1) % Card_DATA.length)
    setFlipped((flipped) => false)
  }
  const prevCard = () => {
    if (index === 0) {
      setIndex(Card_DATA.length - 1)
    }
    else
      setIndex((index) => (index + Card_DATA.length - 1) % Card_DATA.length)
  ...
  return (
    <>
      <Card text={card_text} category={category} image={image} onClick={flipCard}/>
